...
# df = categorias_geral.groupby(['TABELA', 'CATEGORIA'])[['TOTAL', 'PERCENTUAL']].sum().reset_index().sort_values(by=['PERCENTUAL'], ascending=False)
# print(df)

# plt.figure(figsize=(7, 4))
# sns.barplot(df, x="TABELA", y="PERCENTUAL", hue="CATEGORIA", estimator="sum", errorbar=None, palette=['#83C9FF','#0068C9','#FFABAB'])
...
# categorias_geral['PERCENTUAL'] = categorias_geral.groupby('CATEGORIA')['TOTAL'].transform(lambda x: (x / x.sum()) * 100).round(2)
...
# categorias_geral['TABELA'] = categorias_geral['BANCO'].replace(mapeamento)
# print(categorias_geral)
# print()

# plt.figure(figsize=(7, 4))
# sns.barplot(categorias_geral, x="BANCO", y="TOTAL", hue="CATEGORIA", estimator="sum", errorbar=None, palette=CUSTOM_PALETTE)
...
# plt.xlabel('')
# plt.ylabel('')
# plt.ylim(0, categorias_geral['TOTAL'].max() * 1.1)
...
# df = categorias_geral.groupby(['TABELA', 'CATEGORIA'])[['TOTAL', 'PERCENTUAL']].sum().reset_index().sort_values(by=['TOTAL'], ascending=False)
# print(df)

# plt.figure(figsize=(7, 4))
# sns.barplot(df, x="TABELA", y="TOTAL", hue="CATEGORIA", estimator="sum", errorbar=None, palette=CUSTOM_PALETTE)
